...
fn parse_operator_packet(version: u8, bit_reader: &mut BitReader) -> Option<(usize, Packet)> {
...
fn parse_packet(bit_reader: &mut BitReader) -> Option<(usize, Packet)> {
    ...
    if let Some((bits_read, packet)) = match packet_type {
        ...
        _ => parse_operator_packet(version, bit_reader),
    } {
...
fn sum_version_numbers(packet: &Packet) -> usize {
    match packet {
        ...
        Packet::OperatorPacket { version, packets } => (*version as usize) + packets.iter().map(sum_version_numbers).sum::<usize>(),
    }
}
